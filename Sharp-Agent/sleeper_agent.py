...
if __name__ == "__main__":
    ...
    if args.vnet is not None:  # Validate the transfer model given by args.vnet
        ...
        if args.vruns > 0:
            ...
            stats_results = model.validate(
                data, poison_delta, val_max_epoch=args.val_max_epoch
            )
        else:
            stats_results = None
        args.net = train_net
    ...
    print(
        f"--------------------------- craft time: {str(datetime.timedelta(seconds=craft_time - train_time))}"
    )
    print(
        f"--------------------------- test time: {str(datetime.timedelta(seconds=test_time - craft_time))}"
    )
    print("-------------Job finished.-------------------------")
