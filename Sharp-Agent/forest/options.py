...
def options():
    ...
    parser.add_argument(
        "--recipe",
        default="gradient-matching",
        type=str,
        choices=[
            "gradient-matching",
            "gradient-matching-private",
            "gradient-matching-mt",
            "watermark",
            "poison-frogs",
            "metapoison",
            "hidden-trigger",
    ...
    parser.add_argument(
        "--modelkey", default=None, type=int, help="Initialize the model with this key."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=8,
        type=int,
        help="Size of patch to be added to source images",
    )
    # Files and folders
    parser.add_argument(
        "--name",
    ...
    parser.add_argument(
        "--defense_type", default=None, type=str, help="Add custom novel defenses."
    )
    parser.add_argument(
        "--defense_strength",
        default=None,
        type=float,
        help="Add custom strength to novel defenses.",
    )
    parser.add_argument(
        "--defense_steps",
        default=None,
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Which filtering defense to use.",
        choices=["madry", "deepknn", "activation_clustering"],
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Use adversarial steps during poison brewing.",
    )
    parser.add_argument(
        "--pmix",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        help="Loss criterion for source loss",
    )
    parser.add_argument(
        "--restarts", default=1, type=int, help="How often to restart the attack."
    )
    parser.add_argument(
        "--load_patch",
        default="patches/trigger_10.png",
        type=str,
        help="Path to load image for patch attack",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--loss", default="similarity", type=str
    )  # similarity is stronger in  difficult situations
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="How often to re-initialize and check source after retraining",
    )
    parser.add_argument(
        "--vnet",
        default=None,
    ...
    parser.add_argument(
        "--skip_clean_training",
        action="store_true",
        help="Skip clean training. This is only suggested for attacks that do not depend on a clean model.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="Override default epochs of --optimization strategy",
    )
    parser.add_argument(
        "--batch_size",
        default=None,
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Path to benchmarking setup (pickle file)",
    )
    parser.add_argument(
        "--benchmark_idx", default=0, type=int, help="Index of benchmark test"
    )
    ...
    parser.add_argument(
        ...
        default="features",
        type=str,
        help="This can be used when testing defenses.",
    )
    parser.add_argument(
        "--dryrun",
        action="store_true",
        help="This command runs every loop only a single time.",
    )
    parser.add_argument(
        "--save",
        default=None,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Adding patch to sources",
    )
    parser.add_argument(
        "--keep_sources",
        action="store_true",
        default=True,
        help="Removing sources from valid set",
    )
    parser.add_argument(
        "--sources_train", default=5000, type=int, help="Number of sources for training"
    )
    parser.add_argument(
        "--source_gradient_batch",
        default=2500,
        type=int,
        help="Batch size for sources train gradient computing",
    )
    parser.add_argument(
        "--val_max_epoch",
        default=80,
        type=int,
        help="Train only up to this epoch for final validation.",
    )
    parser.add_argument(
        "--retrain_max_epoch",
        default=None,
    ...
    parser.add_argument(
        ...
        default="from-scratch",
        type=str,
        choices=["from-scratch", "finetuning"],
    )
    parser.add_argument(
        "--load_feature_repr",
        action="store_true",
        help="Load feature representation of the model trained on clean data",
    )
    parser.add_argument(
        "--random_patch",
        action="store_true",
        default=True,
        help="Adding patch to sources on a random location",
    )
    parser.add_argument(
        "--retrain_iter",
        default=50,
        type=int,
        help="Start retraining every <retrain_iter> iterations",
    )
    parser.add_argument(
        "--source_selection_strategy",
        default=None,
        type=str,
        choices=["max_gradient", "max_loss"],
        help="Sources_train selection strategy",
    )
    parser.add_argument(
        "--num_source_selection",
        default=2500,
        type=int,
        help="Number of sources (for training) to be selected",
    )
    parser.add_argument(
        "--poison_selection_strategy",
        default="max_gradient",
        type=str,
        choices=["max_gradient"],
        help="Poison selection strategy",
    )
    parser.add_argument(
        "--num_raw_poisons",
        default=5000,
        type=int,
        help="Number of poisons to be selected from",
    )

    # shapness
    parser.add_argument(
        "--savename", default=None, type=str, help="poison data saving name"
    )
    parser.add_argument(
