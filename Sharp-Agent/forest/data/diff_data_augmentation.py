...
class _InvertibleGridSample(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return (
            F.grid_sample(
                grad_output, inverse_grid, align_corners=ctx.align, mode=ctx.mode
            ),
            None,
            None,
            None,
...
class RandomTransformFixed(RandomTransform):
    ...
    def generate_transformations(bs, device=torch.device("cpu")):
        ...
        theta = torch.cat(
            (
                torch.eye(2, device=device).repeat(bs, 1, 1),
                seed.unsqueeze(1).permute(0, 2, 1),
            ),
            dim=2,
        )
        theta[flips, 0, 0] *= -1
...
class _GridShiftTransform(torch.autograd.Function):
    ...
    def forward(ctx, x, trafo):
        ...
        if flip:
            x = x.flip(dims=(3,))
        y[:, :, start[0] : stop[0], start[1] : stop[1]] = x[
    ...
    def backward(ctx, grad_output):
        ...
        grad_input[:, :, start[0] : stop[0], start[1] : stop[1]] = grad_output[
            :, :, start[0] : stop[0], start[1] : stop[1]
        ]
        if flip:
