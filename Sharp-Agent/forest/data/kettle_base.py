...
from .diff_data_augmentation import (
    FlipLR,
    RandomGridShift,
    RandomTransform,
    RandomTransformFixed,
)
...
class _Kettle:
    ...
    def __init__(
        ...
        batch_size,
        augmentations,
        mixing_method=dict(type=None, strength=0.0),
        ...
        self.trainset, self.validset = construct_datasets(
            self.args.dataset, self.args.data_path, NORMALIZE
        )
        if self.args.pretrain_dataset is not None:
            self.pretrain_trainset, self.pretrain_validset = construct_datasets(
                self.args.pretrain_dataset, self.args.data_path, NORMALIZE
            )
        ...
        if self.args.pretrain_dataset is not None:
            self.pretrainloader = torch.utils.data.DataLoader(
                self.pretrain_trainset,
                batch_size=min(self.batch_size, len(self.trainset)),
                shuffle=True,
                drop_last=False,
                num_workers=num_workers,
            ...
            self.prevalidloader = torch.utils.data.DataLoader(
                self.pretrain_validset,
                batch_size=min(self.batch_size, len(self.validset)),
                shuffle=False,
                drop_last=False,
                num_workers=num_workers,
        ...
        if args.ablation < 1.0:
            self.sample = random.sample(
                range(len(self.trainset)), int(self.args.ablation * len(self.trainset))
            )
            self.partialset = Subset(self.trainset, self.sample)
            self.partialloader = torch.utils.data.DataLoader(
                self.partialset,
                batch_size=min(self.batch_size, len(self.partialset)),
                shuffle=True,
                drop_last=False,
                num_workers=num_workers,
                pin_memory=PIN_MEMORY,
        ...
        self.clean_ids = [
            ...
            if self.poison_lookup.get(idx) is None
        ]
        # Finally: status
        self.print_status()
    ...
    def print_status(self):
        ...
        if len(self.source_train_ids) > 5:
            ...
            print(
                f'--Source images assigned target class {", ".join([class_names[i] for i in _target[:5]])}...'
            )
        else:
            print(
            ...
            print(
                f'--Source images assigned target class {", ".join([class_names[i] for i in _target])}.'
            )

        if self.poison_setup["poison_class"] is not None:
            print(
                f'--Poison images drawn from class {class_names[self.poison_setup["poison_class"]]}.'
            )
        else:
        ...
        if self.args.ablation < 1.0:
            print(
                f"--Partialset is {len(self.partialset)/len(self.trainset):2.2%} of full training set"
            )
            num_p_poisons = len(
                np.intersect1d(self.poison_ids.cpu().numpy(), np.array(self.sample))
            )
            print(
                f"--Poisons in partialset are {num_p_poisons} ({num_p_poisons/len(self.poison_ids):2.2%})"
            )
    ...
    def prepare_diff_data_augmentations(self, normalize=True):
        ...
        trainset, validset = construct_datasets(
            self.args.dataset, self.args.data_path, normalize
        )

        # Prepare data mean and std for later:
        if normalize:
            self.dm = torch.tensor(trainset.data_mean)[None, :, None, None].to(
                **self.setup
            )
            self.ds = torch.tensor(trainset.data_std)[None, :, None, None].to(
                **self.setup
            )
        else:
            self.dm = (
                torch.tensor(trainset.data_mean)[None, :, None, None]
                .to(**self.setup)
                .zero_()
            )
            self.ds = (
                torch.tensor(trainset.data_std)[None, :, None, None]
                .to(**self.setup)
                .fill_(1.0)
            )
        ...
        if self.augmentations is not None or self.args.paugment:
            if "CIFAR" in self.args.dataset:
                ...
                params = dict(
                    source_size=28, sourcetarget_size_size=28, shift=4, fliplr=True
                )
            elif "SVHN" in self.args.dataset:  # add svhn
                params = dict(source_size=32, target_size=32, shift=8, fliplr=True)
            elif "TinyImageNet" in self.args.dataset:
                params = dict(
                    source_size=64, target_size=64, shift=64 // 4, fliplr=True
                )
            elif "ImageNet" in self.args.dataset:
                params = dict(
                    source_size=224, target_size=224, shift=224 // 4, fliplr=True
                )
            ...
            if self.augmentations == "default":
                ...
                self.augment = RandomTransformFixed(
                    **params, mode="bilinear", align=False
                )
            elif self.augmentations == "affine-trafo-nn-align":
                self.augment = RandomTransformFixed(
                    **params, mode="nearest", align=False
                )
            ...
            if self.mixing_method["type"] != "" or self.args.pmix:
                if "mixup" in self.mixing_method["type"]:
                    nway = (
                        int(self.mixing_method["type"][0])
                        if "way" in self.mixing_method["type"]
                        else 2
                    )
                    ...
                    raise ValueError(
                        f'Invalid mixing data augmentation {self.mixing_method["type"]} given.'
                    )
        ...
        if mode == "packed":
            ...
            for enum, (source, _, idx) in enumerate(self.sourceset):
                ...
                _save_image(
                    ...
                    target_class,
                    idx,
                    location=os.path.join(path, "sources", names[target_class]),
                    train=False,
                )
            ...
            name_candidate = (
                f"{self.args.name}_{self.args.dataset}T{sourceclass}P{poisonclass}"
            )
            ...
            if mode == "automl-upload":
                ...
                automl_phase = "upload"
            automl_bridge(
                self, poison_delta, name, mode=automl_phase, dryrun=self.args.dryrun
            )
            ...
            sub_path = os.path.join(
                path, "benchmark_results", foldername, str(self.args.benchmark_idx)
            )
            os.makedirs(sub_path, exist_ok=True)

            # Poisons
            benchmark_poisons = []
            for lookup, key in enumerate(
                self.poison_lookup.keys()
            ):  # This is a different order than we usually do for compatibility with the benchmark
            ...
            with open(os.path.join(sub_path, "source.pickle"), "wb+") as file:
                pickle.dump(
                    (_torch_to_PIL(source), source_label),
                    file,
    ...
    def select_poisons(self, victim, selection):
        ...
        if selection == "max_gradient":
            ...
            images = torch.stack([data[0] for data in self.poisonset], dim=0).to(
                **self.setup
            )
            labels = torch.tensor([data[1] for data in self.poisonset]).to(
                device=self.setup["device"], dtype=torch.long
            )
            poison_ids = torch.tensor(
                [data[2].item() for data in self.poisonset], dtype=torch.long
            )
            ...
            if self.args.ensemble == 1:
                ...
                for image, label in zip(images, labels):
                    ...
                    gradients = torch.autograd.grad(
                        loss, differentiable_params, only_inputs=True
                    )
                    grad_norm = 0
            ...
            # ensemble models
                ...
                grad_norms = [
                    sum(col) / float(len(col)) for col in zip(*grad_norms_list)
                ]

            poison_num = int(np.ceil(self.args.budget * len(self.trainset)))
            indices = [i[0] for i in sorted(enumerate(grad_norms), key=lambda x: x[1])][
                -poison_num:
            ]

        else:
            raise NotImplementedError(
                "Poison selection {} strategy is not implemented yet!".format(selection)
            )
