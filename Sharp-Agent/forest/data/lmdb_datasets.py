...
class LMDBDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, database_path="", name="", rebuild_cache=False):
        ...
        if os.path.isfile(database_path):
            raise ValueError(
                "LMDB path must lead to a folder containing the databases."
            )
        self.dataset = dataset
        self.path = os.path.join(
            os.path.expanduser(database_path), f"{type(dataset).__name__}_{name}.lmdb"
        )
        ...
        if os.path.isfile(self.path):
            ...
            checksum = create_database(
                self.dataset, self.path, mean=self.data_mean, std=self.data_std
            )
        ...
        with self.db.begin(write=False) as txn:
            try:
                ...
                raise ValueError(
                    f"The provided LMDB dataset at {self.path} is unfinished or damaged."
                )
    ...
    def __len__(self):
        """Draw length from source dataset."""
        return self.length
    ...
    def __getitem__(self, index):
        ...
        np_img = np.reshape(
            np.frombuffer(buffer.read(), dtype=np.uint8), self.img_shape
        )
        img = torch.as_tensor(np_img, dtype=torch.float) / 255
        img = torchvision.transforms.functional.normalize(
            img, self.data_mean, self.data_std, inplace=True
        )

        # load label and id
        source, idx = self.dataset.get_target(index)

        return img, source, idx

    def get_target(self, index):
        """Return only the pair (source, idx)."""
        return self.dataset.get_target(index)


def create_database(
    dataset, database_path, write_frequency=5000, mean=(0, 0, 0), std=(1, 1, 1)
):
    ...
    if platform.system() == "Linux":
        ...
        raise ValueError(
            "Provide a reasonable default map_size for your operating system."
        )
    db = lmdb.open(
        database_path,
        subdir=False,
    ...
    for idx, (img, source, idx) in enumerate(dataset):
        # unnormalize
        img_uint8 = (_unnormalize(img, mean, std).mul(255).add_(0.5).clamp_(0, 255)).to(
            torch.uint8
        )
        # serialize
