...
class KettleExternal(_Kettle):
    ...
    def __init__(
        ...
        if "xtrain" in data_package.keys():
            ...
            self.trainset, self.validset, self.poisonset, self.sourceset = (
                self._load_metapoison_files(data_package)
            )
    ...
    def _load_metapoison_files(self, data_package):
        ...
        possible_poison_ids = torch.arange(len(data_package["xtrain"]))[
            data_package["ytrain"] == data_package["ysourceadv"]
        ]

        if NORMALIZE:
            data_mean, data_std = [m * 255 for m in cifar10_mean], [
                s * 255 for s in cifar10_std
            ]

        for key in ["xtrain", "xvalid", "xsource"]:
            if NORMALIZE:
                dm, ds = (
                    torch.as_tensor(data_mean)[None, :, None, None],
                    torch.as_tensor(data_std)[None, :, None, None],
                )
                data_package[key] = (
                    torch.as_tensor(data_package[key])
                    .permute(0, 3, 1, 2)
                    .sub_(dm)
                    .div_(ds)
                )
                ...
                data_package[key] = (
                    torch.as_tensor(data_package[key]).permute(0, 3, 1, 2).div_(255)
                )

        ids = torch.arange(len(data_package["xtrain"]))
        trainset = torch.utils.data.TensorDataset(
            data_package["xtrain"], torch.as_tensor(data_package["ytrain"]), ids
        )
        validset = torch.utils.data.TensorDataset(
            data_package["xvalid"],
            torch.as_tensor(data_package["yvalid"]),
            ids[: len(data_package["yvalid"])],
        )
        poisonset = torch.utils.data.TensorDataset(
            data_package["xtrain"][possible_poison_ids],
            torch.as_tensor(data_package["ytrain"])[possible_poison_ids],
            possible_poison_ids,
        )
        sourceset = torch.utils.data.TensorDataset(
            data_package["xsource"],
            torch.as_tensor(data_package["ysourceadv"]),
            torch.arange(len(data_package["xsource"])),
        )

        self.poison_setup = dict(
            poison_budget=self.args.budget,
            source_num=len(data_package["xsource"]),
            poison_class=data_package["ysourceadv"][0],
            source_class=data_package["ysource"][0],
