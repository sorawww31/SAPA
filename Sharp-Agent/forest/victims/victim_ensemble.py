...
class _VictimEnsemble(_VictimBase):
    ...
    def initialize(self, pretrain=False, seed=None):
        ...
        for idx in range(self.args.ensemble):
            ...
            model, defs, optimizer, scheduler = self._initialize_model(
                model_name, pretrain=pretrain
            )
        ...
        for idx in range(self.args.ensemble):
            ...
            if not keep_last_layer:
                ...
                replacement_model = get_model(
                    model_name, self.args.dataset, pretrained=self.args.pretrained_model
                )

                # Rebuild model with new last layer
                frozen = self.models[idx].frozen
                self.models[idx] = torch.nn.Sequential(
                    *list(self.models[idx].children())[:-1],
                    torch.nn.Flatten(),
            ...
            self.optimizers[idx], self.schedulers[idx] = get_optimizers(
                self.models[idx], self.args, self.definitions[idx]
            )
    ...
    def _iterate(self, kettle, poison_delta, max_epoch=None, pretraining_phase=False):
        ...
        if (
            poison_delta is None
            and self.args.stagger is not None
            and not pretraining_phase
        ):
            if self.args.stagger == "firstn":
                ...
                stagger_list = [
                    int(epoch)
                    for epoch in np.linspace(0, max_epoch, self.args.ensemble)
                ]
            elif self.args.stagger == "inbetween":
                stagger_list = [
                    int(epoch)
                    for epoch in np.linspace(0, max_epoch, self.args.ensemble + 2)[1:-1]
                ]
            else:
        ...
        for idx, single_model in enumerate(zip(*multi_model_setup)):
            ...
            for epoch in range(stagger_list[idx]):
                self._step(
                    kettle, poison_delta, epoch, stats, *single_model, pretraining_phase
                )
                if self.args.dryrun:
                    break
            # Return to CPU
    ...
    def step(self, kettle, poison_delta, poison_sources, true_classes):
        ...
        for idx, single_model in enumerate(zip(*multi_model_setup)):
            ...
            if torch.cuda.device_count() > 1:
                ...
                model.frozen = model.module.frozen
            self._step(
                kettle, poison_delta, self.epochs[idx], defaultdict(list), *single_model
            )
            self.epochs[idx] += 1
            ...
            (
                ...
                self.optimizers[idx],
                self.schedulers[idx],
            ) = (model, defs, optimizer, scheduler)
        ...
        for model in self.models:
            with GPUContext(self.setup, model) as model:
                ...
                if selection == "max_gradient":
                    ...
                    indices = [
                        i[0] for i in sorted(enumerate(grad_norms), key=lambda x: x[1])
                    ][-self.args.num_source_selection :]
                    images = images[indices]
                    labels = labels[indices]
                    print(
                        "{} sources with maximum gradients selected".format(
                            self.args.num_source_selection
                        )
                    )
                ...
                if not self.args.source_gradient_batch == None:
                    ...
                    if images.shape[0] < batch_size:
                        ...
                        if images.shape[0] % batch_size != 0:
                            batch_size = images.shape[0] // ceil(
                                images.shape[0] / batch_size
                            )
                            warnings.warn(
                                f"Batch size changed to {batch_size} to fit source train size"
                            )
                    gradients = None
                    for i in range(images.shape[0] // batch_size):
                        loss = batch_size * criterion(
                            model(images[i * batch_size : (i + 1) * batch_size]),
    ...
    def compute(self, function, *args):
        ...
        for idx, (model, optimizer) in enumerate(zip(self.models, self.optimizers)):
            with GPUContext(self.setup, model) as model:
                single_arg = [
                    arg[idx] if hasattr(arg, "__iter__") else arg for arg in args
                ]
                outputs.append(function(model, optimizer, *single_arg))
        # collate
        avg_output = [
            np.mean([output[idx] for output in outputs])
            for idx, _ in enumerate(outputs[0])
        ]
        return avg_output
