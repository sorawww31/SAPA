...
def get_model(model_name, dataset_name, pretrained=False):
    ...
    if (
        "CIFAR" in dataset_name or "MNIST" in dataset_name or "SVHN" in dataset_name
    ):  # add svhn:
        if pretrained:
            raise ValueError(
        ...
        if "ResNet" in model_name:
            ...
            model = ConvNetBN(
                width=32, in_channels=in_channels, num_classes=num_classes
            )
        elif model_name == "Linear":
            model = linear_model(dataset_name, num_classes=num_classes)
        elif model_name == "alexnet-mp":
            model = alexnet_metapoison(
                in_channels=in_channels, num_classes=num_classes, batchnorm=False
            )
        elif model_name == "alexnet-mp-bn":
            model = alexnet_metapoison(
                in_channels=in_channels, num_classes=num_classes, batchnorm=True
            )
        elif "VGG" in model_name:
            model = VGG(model_name, in_channels=in_channels, num_classes=num_classes)
        elif model_name == "MobileNetV2":
            model = MobileNetV2(
                num_classes=num_classes, train_dp=0, test_dp=0, droplayer=0, bdp=0
            )
        elif model_name == "vit":
            model = ViT(
                image_size=int(32),
        ...
        if "efficientnet-b" in model_name.lower():
            ...
            try:
                model = getattr(torchvision.models, model_name.lower())(
                    pretrained=pretrained, **extra_args
                )
...
def linear_model(dataset, num_classes=10):
    ...
    return torch.nn.Sequential(
        torch.nn.Flatten(), torch.nn.Linear(dimension, num_classes)
    )
    ...
    model = torch.nn.Sequential(
        OrderedDict(
            [
                (
                    "conv0",
                    torch.nn.Conv2d(in_channels, 1 * width, kernel_size=3, padding=1),
                ),
                ("relu0", torch.nn.ReLU()),
                (
                    "conv1",
...
def alexnet_metapoison(
    widths=[16, 32, 32, 64, 64], in_channels=3, num_classes=10, batchnorm=False
):
    ...
    def convblock(width_in, width_out):
        ...
        return torch.nn.Sequential(
            torch.nn.Conv2d(width_in, width_out, kernel_size=3, padding=1),
            torch.nn.ReLU(),
            bn,
    ...
    model = torch.nn.Sequential(
        *blocks, torch.nn.Flatten(), torch.nn.Linear(widths[-1], num_classes)
    )
    return model
            ...
            OrderedDict(
                [
                    (
                        "conv0",
                        torch.nn.Conv2d(
                            in_channels, 1 * width, kernel_size=3, padding=1
                        ),
                    ),
                    (
                        "bn0",
                        torch.nn.BatchNorm2d(
                            1 * width, momentum=0.1, track_running_stats=True
                        ),
                    ),
                    ("relu0", torch.nn.ReLU()),
                    (
                    ...
                    (
                        "bn1",
                        torch.nn.BatchNorm2d(
                            2 * width, momentum=0.1, track_running_stats=True
                        ),
                    ),
                    ("relu1", torch.nn.ReLU()),
                    (
                    ...
                    (
                        "bn3",
                        torch.nn.BatchNorm2d(
                            4 * width, momentum=0.1, track_running_stats=True
                        ),
                    ),
                    ("relu3", torch.nn.ReLU()),
