...
def construct_attack(
    novel_defense, model, loss_fn, dm, ds, tau, init, optim, num_classes, setup
):
    """Interface for this submodule."""
    eps = novel_defense[
        "strength"
    ]  # The defense parameter encodes the eps bound used during training
    if "adversarial-evasion" in novel_defense["type"]:
        return AdversarialAttack(
            model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup
        )
    elif "adversarial-wb" in novel_defense["type"]:
        return AlignmentPoisoning(
            model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup
        )
    elif "adversarial-se" in novel_defense["type"]:
        return MatchingPoisoning(
            model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup
        )
    elif "adversarial-mp" in novel_defense["type"]:
        return MetaPoisoning(
            model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup
        )
    elif (
        "adversarial-fc" in novel_defense["type"]
        or "adversarial-cp" in novel_defense["type"]
    ):
        return FeatureCollisionPoisoning(
            model, loss_fn, dm, ds, tau, eps, init, optim, num_classes, setup
        )
        ...
        return RandomAttack(
            model, loss_fn, dm, ds, tau, eps, "laplacian", optim, num_classes, setup
        )
    elif "adversarial-bernoulli" in novel_defense["type"]:
        return RandomAttack(
            model, loss_fn, dm, ds, tau, eps, "bernoulli", optim, num_classes, setup
        )
        ...
        raise ValueError(
            f'Invalid adversarial training objective specified: {novel_defense["type"]}.'
        )
...
class BaseAttack(torch.nn.Module):
    ...
    def attack(
        ...
        for step in range(steps):
            ...
            loss = self._objective(
                inputs + delta, labels, temp_sources, temp_fake_labels
            )
            (delta.grad,) = torch.autograd.grad(
                loss, delta, retain_graph=False, create_graph=False, only_inputs=True
            )
            # Optim step
            ...
            with torch.no_grad():
                ...
                delta.data = torch.max(
                    torch.min(delta, (1 - self.dm) / self.ds - inputs),
                    -self.dm / self.ds - inputs,
                )
    ...
    def _init_perturbation(self, input_shape):
        if self.init == "zero":
            delta = torch.zeros(
                input_shape, device=self.setup["device"], dtype=self.setup["dtype"]
            )
        elif self.init == "rand":
            delta = (
                torch.rand(
            ...
            scale = torch.as_tensor(
                self.eps / self.ds / 255, device=self.setup["device"]
            ).mean()
        ...
        delta.data = torch.max(
            torch.min(delta, self.eps / self.ds / 255), -self.eps / self.ds / 255
        )
...
class WatermarkPoisoning(BaseAttack):
    ...
    def attack(
        ...
        if num_sources == num_inputs:
            ...
            delta = (
                temp_sources.repeat(num_inputs // num_sources, 1, 1, 1)[:num_inputs]
                - inputs
            )
        else:
            factor = num_sources // num_inputs
            delta = (
                temp_sources[: (factor * num_sources)]
                .reshape(num_inputs, -1, *img_shape)
                .mean(dim=1)
                - inputs
            )
        delta *= self.eps / self.ds / 255
...
class AlignmentPoisoning(BaseAttack):
    ...
    def _objective(self, inputs, labels, temp_sources, temp_fake_labels):
        ...
        poison_grad = torch.autograd.grad(
            poison_loss, differentiable_params, retain_graph=True, create_graph=True
        )

        source_loss = self.loss_fn(self.model(temp_sources), temp_fake_labels)
        source_grad = torch.autograd.grad(
            source_loss, differentiable_params, retain_graph=True, create_graph=True
        )
...
class MatchingPoisoning(BaseAttack):
    ...
    def _objective(self, inputs, labels, temp_sources, temp_fake_labels):
        ...
        return (
            objective / tnorm.sqrt()
        )  # tgrad is a constant normalization factor as in witch_matching
...
class MetaPoisoning(BaseAttack):
    ...
    def _objective(self, inputs, labels, temp_sources, temp_fake_labels):
        ...
        with higher.innerloop_ctx(
            self.model, optimizer, copy_initial_weights=False
        ) as (fmodel, fopt):
...
class HTBD(BaseAttack):
    ...
    def attack(
        ...
        for step in range(steps):
            ...
            # Gradient step
            loss = self._objective(
                inputs + delta, temp_sources, input_indcs, source_indcs
            )
            (delta.grad,) = torch.autograd.grad(
                loss, delta, retain_graph=False, create_graph=False, only_inputs=True
            )
    ...
    def _apply_patch(self, temp_sources):
        patch_shape = [
            [
                3,
                random.randint(
                    int(0.2 * temp_sources.shape[2]), int(0.4 * temp_sources.shape[2])
                ),
                random.randint(
                    int(0.1 * temp_sources.shape[3]), int(0.2 * temp_sources.shape[3])
                ),
            ]
            for _ in range(temp_sources.shape[0])
        ]
        ...
        for i in range(len(patch)):
            temp_sources[
                i,
                :,
                x_locations[i] : x_locations[i] + patch_shape[i][1],
    ...
    def _index_mapping(self, inputs, temp_sources):
        with torch.no_grad():
            ...
            for _ in range(feat_source.size(0)):
                dist_min_index = (
                    (dist == torch.min(dist)).nonzero(as_tuple=False).squeeze()
                )
    ...
    def _create_patch(self, patch_shape):
        ...
        for i in range(len(patch_shape)):
            temp_patch = 0.5 * torch.ones(
                patch_shape[i][0], patch_shape[i][1], patch_shape[i][2]
            )
...
class PatchAttack(BaseAttack):
    ...
    def attack(
        ...
        patch_shape = [
            [
                3,
                random.randint(int(0.1 * inputs.shape[2]), int(0.4 * inputs.shape[2])),
                random.randint(int(0.1 * inputs.shape[3]), int(0.4 * inputs.shape[3])),
            ]
            for _ in range(self.num_classes)
        ]
        # patch_shape = [[3, self.eps, self.eps] for _ in range(self.num_classes)]

        x_locations, y_locations = self._set_locations(
            inputs.shape, labels, patch_shape
        )
        ...
        for i in range(delta1.shape[0]):
            ...
            delta1[
                i,
                :,
                x_locations[i] : x_locations[i] + patch_shape[temp_label][1],
                y_locations[i] : y_locations[i] + patch_shape[temp_label][2],
            ] = (
                patch[temp_label]
                - inputs[
                    i,
        ...
        temp_source_labels = [
            permute_list[temp_true_label] for temp_true_label in temp_true_labels
        ]
        x_locations, y_locations = self._set_locations(
            temp_sources.shape, temp_source_labels, patch_shape
        )
    ...
    def _set_locations(self, input_shape, labels, patch_shape):
        ...
        for i in range(input_shape[0]):
            x_locations.append(
                random.randint(0, input_shape[2] - patch_shape[labels[i]][1])
            )
            y_locations.append(
                random.randint(0, input_shape[3] - patch_shape[labels[i]][2])
            )
        return x_locations, y_locations
