...
class _VictimSingle(_VictimBase):
        ...
        if not self.args.source_gradient_batch == None:
            ...
            for i in range(images.shape[0] // batch_size):
                loss = batch_size * criterion(
                    self.model(images[i * batch_size : (i + 1) * batch_size]),
    ...
    def worst_sharp_grad(
        self, images, labels, criterion=None, selection=None, sigma=0.05
    ):
        import copy
        ...
        if not self.args.source_gradient_batch == None:
            ...
            for i in range(images.shape[0] // batch_size):
                loss = batch_size * criterion(
                    net_clone(images[i * batch_size : (i + 1) * batch_size]),
                    labels[i * batch_size : (i + 1) * batch_size],
                )
                if i == 0:
                    gradients = torch.autograd.grad(
                        loss, net_clone.parameters(), only_inputs=True
                    )
                else:
                    gradients = tuple(
                        map(
        ...
        if not self.args.source_gradient_batch == None:
            ...
            for i in range(images.shape[0] // batch_size):
                if i == 0:
                    ...
                    loss_s = batch_size * criterion(
                        output_p, labels[i * batch_size : (i + 1) * batch_size]
                    )
                    gradients = torch.autograd.grad(
                    ...
                    gradients_m = torch.autograd.grad(
                        loss_s, net_clone.parameters(), only_inputs=True
                    )
                    for _ in range(19):
                        net_clone = copy.deepcopy(self.model)
