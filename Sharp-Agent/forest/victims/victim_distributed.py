...
class _VictimDistributed(_VictimSingle):
    ...
    def __init__(
        self, args, defs, setup=dict(device=torch.device("cpu"), dtype=torch.float)
    ):
        ...
        if self.args.world_size < len(self.args.net):
            raise ValueError(
                f"More models requested than distr. ensemble size."
                f"Launch more instances or reduce models."
            )
        if self.args.ensemble > 1:
            if self.args.ensemble != self.args.world_size:
                raise ValueError(
                    "The ensemble option is disregarded in distributed mode. One model will be launched per instance."
                )
    ...
    def initialize(self, pretrain=False, seed=None):
        if self.args.modelkey is None:
            if seed is None:
                init_seed = torch.randint(
                    0, 2**32 - 128, [1], device=self.setup["device"]
                )
            else:
                init_seed = torch.as_tensor(
                    seed, dtype=torch.int64, device=self.setup["device"]
                )
        else:
            init_seed = torch.as_tensor(
                self.args.modelkey, dtype=torch.int64, device=self.setup["device"]
            )
        ...
        print(
            f"Model {model_name} initialized with random key {self.model_init_seed} on rank {self.rank}."
        )
    ...
    def reinitialize_last_layer(self, reduce_lr_factor=1.0, seed=None):
        ...
        print(
            f"Model {model_name} last layer reinitialized with random key {self.model_init_seed} on rank {self.rank}."
        )
        print(repr(self.defs))
    ...
    def _iterate(self, kettle, poison_delta, max_epoch=None, pretraining_phase=False):
        ...
        if (
            ...
            if self.args.stagger == "firstn":
                ...
                stagger_list = [
                    int(epoch) for epoch in np.linspace(0, max_epoch, self.world_size)
                ]
            elif self.args.stagger == "inbetween":
                stagger_list = [
                    int(epoch)
                    for epoch in np.linspace(0, max_epoch, self.world_size + 2)[1:-1]
                ]
            else:
        ...
        if self.args.stagger is not None and poison_delta is None:
            ...
            torch.distributed.reduce(
                stat_block, dst=0, op=torch.distributed.ReduceOp.SUM
            )
    ...
    def distributed_control(self, inputs, labels, poison_slices, batch_positions):
        ...
        if batch_positions is None:
            ...
            batch_positions = torch.as_tensor(
                batch_positions, device=self.setup["device"]
            )
            torch.distributed.broadcast(batch_positions, src=0)
