...
def bypass_last_layer(model):
    ...
    headless_model = torch.nn.Sequential(
        *(layer_cake[:-1]), torch.nn.Flatten()
    )  # this works most of the time all of the time :<
...
def cw_loss(outputs, target_classes, clamp=-100):
    ...
    target_logits = torch.stack(
        [outputs[i, target_classes[i]] for i in range(outputs.shape[0])]
    )
...
def record_results(
    ...
    args.name = f"{args.budget}_{args.eps}_{args.patch_size}_{args.attackiter}_{args.sources_train}_{args.retrain_scenario}_{args.scenario}_{args.load_feature_repr}_{args.name}"

    save_to_table(
        args.table_path,
        f'{"_".join(args.net)}_{args.threatmodel}_{args.name}',
        args.dryrun,
        budget=args.budget,
        eps=args.eps,
        source=(
            class_names[kettle.poison_setup["source_class"]]
            if kettle.poison_setup["source_class"] is not None
            else "Several"
        ),
        target=class_names[kettle.poison_setup["target_class"][0]],
        poison=(
            class_names[kettle.poison_setup["poison_class"]]
            if kettle.poison_setup["poison_class"] is not None
            else "All"
        ),
        patch=args.load_patch,
        craft_scenario=args.retrain_scenario,
        valid_scenario=args.scenario,
        ...
        source_loss_rerun=_maybe(stats_rerun, "source_losses"),
        source_acc_rerun=_maybe(stats_rerun, "source_accs"),
        brewed_loss=brewed_loss,
        optim=args.attackoptim,
        iter=args.attackiter,
        loss=args.loss,
        tau=args.tau,
        centreg=args.centreg,
        normreg=args.normreg,
        repel=args.repel,
        init=args.init,
        pbatch=args.pbatch,
        ens=args.ensemble,
        vruns=args.vruns,
        pshuffle=args.pshuffle,
        paugment=args.paugment,
        step=args.step,
        full_data=args.full_data,
        stagger=args.stagger,
        maxepoch=args.max_epoch,
        nadapt=args.nadapt,
        data_aug=defs.augmentations,
        epochs=defs.epochs,
        lr=defs.lr,
        poison_loss=args.source_criterion,
        restarts=args.restarts,
        step_decay=args.scheduling,
        ablation=args.ablation,
        benchmark_idx=args.benchmark_idx,
        source_mloss_reinit=_maybe(stats_results, "source_losses", mean=True),
        source_macc_reinit=_maybe(stats_results, "source_accs", mean=True),
        source_mloss_rerun=_maybe(stats_rerun, "source_losses", mean=True),
        source_macc_rerun=_maybe(stats_rerun, "source_accs", mean=True),
        triggered_source_loss_reinit=_maybe(stats_results, "source_losses_clean"),
        triggered_source_acc_reinit=_maybe(stats_results, "source_accs_clean"),
        triggered_source_loss_rerun=_maybe(stats_rerun, "source_losses_clean"),
        triggered_source_acc_rerun=_maybe(stats_rerun, "source_accs_clean"),
        **extra_stats,
        ...
        **{f"ARG_{key}": val for key, val in args.__dict__.items()},
        val_acc_target_clean=_maybe(stats_clean, "valid_accs_target"),
        val_acc_source_clean=_maybe(stats_clean, "valid_accs_source"),
        val_acc_target_rerun=_maybe(stats_rerun, "valid_accs_target"),
        val_acc_source_rerun=_maybe(stats_rerun, "valid_accs_source"),
        val_acc_target_reinit=_maybe(stats_results, "valid_accs_target"),
        val_acc_source_reinit=_maybe(stats_results, "valid_accs_source"),
    )
...
def automl_bridge(kettle, poison_delta, name, mode="poison-upload", dryrun=False):
    ...
    setup = dict(
        uid=name,
        project_id="silent-venture-269920",
        multilabel=False,
        format=".png",
        bucketname="silent-venture-269920-vcm",
        display_name=name,
        dataset_id=None,
        model_id=None,
        ntrial=1,
        mode=mode,
        base_dataset=(
            "ImageNet" if kettle.args.dataset == "ImageNet1k" else kettle.args.dataset
        ),
        dryrun=dryrun,
    )
