...
def _get_poisoned_features(kettle, victim, poison_delta, dryrun=False):
    ...
    with torch.no_grad():
        for i, (img, source, idx) in enumerate(kettle.trainset):
            ...
            if (
                dryrun and i == 3
            ):  # use a few values to populate these adjancency matrices
                break
...
def _DeepKNN(kettle, victim, poison_delta, overestimation_factor=2.0):
    ...
    num_poisons_expected = (
        int(overestimation_factor * kettle.args.budget * len(kettle.trainset))
        if not kettle.args.dryrun
        else 0
    )
    feats, _ = _get_poisoned_features(
        kettle, victim, poison_delta, dryrun=kettle.args.dryrun
    )

    feats = torch.stack(feats, dim=0)
    dist_matrix = torch.zeros((len(feats), len(feats)))
    for i in range(dist_matrix.shape[0]):
        temp_matrix = torch.stack(
            [feats[i] for _ in range(dist_matrix.shape[1])], dim=0
        )
...
def _MadryDefense(kettle, victim, poison_delta, overestimation_factor=1.5):
    ...
    feats, class_indices = _get_poisoned_features(
        kettle, victim, poison_delta, dryrun=kettle.args.dryrun
    )
...
def _ActivationClustering(kettle, victim, poison_delta, clusters=2):
    ...
    for i in range(len(class_indices)):
        if len(class_indices[i]) > 1:
            temp_feats = np.array(
                [
                    feats[temp_idx].squeeze(0).cpu().numpy()
                    for temp_idx in class_indices[i]
                ]
            )
