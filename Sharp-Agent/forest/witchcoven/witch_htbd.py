...
class WitchHTBD(_Witch):
    def _run_trial(self, victim, kettle):
        ...
        for step in range(self.args.attackiter):
            ...
            for batch, example in enumerate(dataloader):
                ...
                indcs = random.sample(
                    list(range(len(self.temp_sourceset))), validated_batch_size
                )
                for i in indcs:
                ...
                loss, prediction = self._batched_step(
                    poison_delta, poison_bounds, example, victim, kettle, sources
                )
                source_losses += loss
            ...
            if step % (self.args.attackiter // 5) == 0 or step == (
    ...
    def _batched_step(
        self, poison_delta, poison_bounds, example, victim, kettle, sources
    ):
        ...
        if len(batch_positions) > 0:
            ...
            if self.args.padversarial is not None:
                delta = self.attacker.attack(
                    inputs.detach(), labels, None, None, steps=5
                )  # the 5-step here is FOR TESTING ONLY
    ...
    def patch_sources(self, kettle):
        if self.args.load_patch == "":
            ...
            patch = self._create_patch(
                [3, int(self.args.patch_size), int(self.args.patch_size)]
            )
        else:
