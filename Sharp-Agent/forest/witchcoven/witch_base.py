...
class _Witch:
    ...
    def __init__(self, args, setup=dict(device=torch.device("cpu"), dtype=torch.float)):
        ...
        self.retain = (
            True if self.args.ensemble > 1 and self.args.local_rank is None else False
        )
            ...
            if len(kettle.sourceset) > 0:
                if self.args.eps > 0:
                    if self.args.budget > 0:
                        ...
                        warnings.warn(
                            "No poison budget given. Nothing can be poisoned."
                        )
                else:
                    poison_delta = kettle.initialize_poison(initializer="zero")
                    warnings.warn(
                        "Perturbation interval is empty. Nothing can be poisoned."
                    )
            else:
                poison_delta = kettle.initialize_poison(initializer="zero")
    ...
    def _brew(self, victim, kettle):
        ...
        print(
            f"Poisons with minimal source loss {self.stat_optimal_loss:6.4e} selected."
        )
    ...
    def _initialize_brew(self, victim, kettle):
        ...
        self.sources = torch.stack([data[0] for data in kettle.sourceset], dim=0).to(
            **self.setup
        )
        self.target_classes = torch.tensor(kettle.poison_setup["target_class"]).to(
            device=self.setup["device"], dtype=torch.long
        )
        self.true_classes = torch.tensor([data[1] for data in kettle.sourceset]).to(
            device=self.setup["device"], dtype=torch.long
        )

        self.sources_train = torch.stack(
            [data[0] for data in kettle.source_trainset], dim=0
        ).to(**self.setup)
        self.sources_train_target_classes = torch.tensor(
            [kettle.poison_setup["target_class"][0]] * kettle.source_train_num
        ).to(device=self.setup["device"], dtype=torch.long)
        self.sources_train_true_classes = torch.tensor(
            [data[1] for data in kettle.source_trainset]
        ).to(device=self.setup["device"], dtype=torch.long)
        ...
        if self.args.source_criterion in ["cw", "carlini-wagner"]:
            self.source_grad, self.source_gnorm = victim.gradient(
                _sources,
                _target_classes,
                cw_loss,
                selection=self.args.source_selection_strategy,
            )
        elif self.args.source_criterion in ["unsourceed-cross-entropy", "unxent"]:
            self.source_grad, self.source_gnorm = victim.gradient(
                _sources, _true_classes, selection=self.args.source_selection_strategy
            )
            for grad in self.source_grad:
                grad *= -1
        elif self.args.source_criterion in ["xent", "cross-entropy"]:
            self.source_grad, self.source_gnorm = victim.gradient(
                _sources, _target_classes, selection=self.args.source_selection_strategy
            )
        elif self.args.source_criterion in ["sharpness"]:
            criterion = nn.CrossEntropyLoss()
            self.source_grad, self.source_gnorm = victim.sharp_grad(
                _sources,
                _target_classes,
                criterion=criterion,
                selection=self.args.source_selection_strategy,
                sigma=self.args.sharpsigma,
            )
        elif self.args.source_criterion in ["worstsharp"]:
            criterion = nn.CrossEntropyLoss()
            self.source_grad, self.source_gnorm = victim.worst_sharp_grad(
                _sources,
                _target_classes,
                criterion=criterion,
                selection=self.args.source_selection_strategy,
        ...
        if self.args.padversarial is not None:
            if not isinstance(victim, _VictimSingle):
                raise ValueError(
                    "Test variant only implemented for single victims atm..."
                )
            attack = dict(
                type=self.args.padversarial, strength=self.args.defense_strength
            )
            self.attacker = construct_attack(
                attack,
                victim.model,
                victim.loss_fn,
                kettle.dm,
    ...
    def _run_trial(self, victim, kettle):
        ...
        if self.args.attackoptim in ["Adam", "signAdam", "momSGD", "momPGD"]:
            ...
            if self.args.attackoptim in ["Adam", "signAdam"]:
                att_optimizer = torch.optim.Adam(
                    [poison_delta], lr=self.tau0, weight_decay=0
                )
            else:
                att_optimizer = torch.optim.SGD(
                    [poison_delta], lr=self.tau0, momentum=0.9, weight_decay=0
                )
            if self.args.scheduling:
                scheduler = torch.optim.lr_scheduler.MultiStepLR(
                    att_optimizer,
                    milestones=[
                        self.args.attackiter // 2.667,
                        self.args.attackiter // 1.6,
                        self.args.attackiter // 1.142,
                    ],
                    gamma=0.1,
                )
            poison_delta.grad = torch.zeros_like(poison_delta)
            dm, ds = kettle.dm.to(device=torch.device("cpu")), kettle.ds.to(
        ...
        for step in range(self.args.attackiter):
            ...
            for batch, example in enumerate(dataloader):
                loss, prediction = self._batched_step(
                    poison_delta, poison_bounds, example, victim, kettle
                )
                source_losses += loss
            ...
            if self.args.attackoptim in ["Adam", "signAdam", "momSGD", "momPGD"]:
                ...
                with torch.no_grad():
                    ...
                    poison_delta.data = torch.max(
                        torch.min(poison_delta, self.args.eps / ds / 255),
                        -self.args.eps / ds / 255,
                    )
                    poison_delta.data = torch.max(
                        torch.min(poison_delta, (1 - dm) / ds - poison_bounds),
                        -dm / ds - poison_bounds,
                    )

            source_losses = source_losses / (batch + 1)
            poison_acc = poison_correct / len(dataloader.dataset)
            if step % (self.args.attackiter // 25) == 0 or step == (
                self.args.attackiter - 1
            ):
                print(
                    f"Iteration {step}: Source loss is {source_losses:2.4f}, "
            ...
            if not self.args.retrain_scenario == None:
                if (
                    step % (self.args.retrain_iter) == 0
                    and step != 0
                    and step != (self.args.attackiter - 1)
                ):
                    ...
                    if self.args.retrain_scenario == "from-scratch":
                        ...
                        victim.reinitialize_last_layer(
                    ...
                    victim._iterate(
                        ...
                        max_epoch=self.args.retrain_max_epoch,
                    )
    ...
    def _batched_step(self, poison_delta, poison_bounds, example, victim, kettle):
        ...
        inputs, labels, poison_slices, batch_positions, randgen = (
            victim.distributed_control(inputs, labels, poison_slices, batch_positions)
        )
        ...
        if len(batch_positions) > 0:
            ...
            if self.args.pmix:
                if (
                    "mix" in victim.defs.mixing_method["type"]
                ):  # this covers mixup, cutmix 4waymixup, maxup-mixup
                    try:
            ...
            if self.args.padversarial is not None:
                ...
                [temp_sources, inputs, temp_true_labels, labels, temp_fake_label] = (
                    _split_data(
                        inputs,
                        labels,
                        source_selection=victim.defs.novel_defense["source_selection"],
                    )
                )
                delta, additional_info = self.attacker.attack(
                    inputs.detach(),
                    labels,
                    temp_sources,
                    temp_fake_label,
            ...
            if self.args.pmix:
                ...
                def criterion(outputs, labels):
                    loss, pred = kettle.mixer.corrected_loss(
            ...
            loss, prediction = victim.compute(
                closure, self.source_grad, self.source_clean_grad, self.source_gnorm
            )
            ...
            # Update Step
            if self.args.attackoptim in ["PGD", "GD"]:
                delta_slice = self._pgd_step(
                    delta_slice, poison_images, self.tau0, kettle.dm, kettle.ds
                )

                # Return slice to CPU:
                poison_delta[poison_slices] = delta_slice.detach().to(
                    device=torch.device("cpu")
                )
            elif self.args.attackoptim in ["Adam", "signAdam", "momSGD", "momPGD"]:
                poison_delta.grad[poison_slices] = delta_slice.grad.detach().to(
                    device=torch.device("cpu")
                )
                poison_bounds[poison_slices] = poison_images.detach().to(
                    device=torch.device("cpu")
                )
            else:
    ...
    def _pgd_step(self, delta_slice, poison_imgs, tau, dm, ds):
        ...
        with torch.no_grad():
            ...
            # Projection Step
            delta_slice.data = torch.max(
                torch.min(delta_slice, self.args.eps / ds / 255),
                -self.args.eps / ds / 255,
            )
            delta_slice.data = torch.max(
                torch.min(delta_slice, (1 - dm) / ds - poison_imgs),
                -dm / ds - poison_imgs,
            )
        return delta_slice
